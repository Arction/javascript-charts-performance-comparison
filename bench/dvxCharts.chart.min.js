/*
 * dvxCharts v5.0.0.0 (http://www.dvxcharts.com)
 * Copyright 2020 dvxCharts Inc. All rights reserved.
 *
...
el.setAttribute(att,value);};jFunc.stop=function(){};jFunc.bind=function(el,eventName,func){el.addEventListener(eventName,func);};jFunc.unbind=function(el,eventName,func){el.removeEventListener(eventName,func);};jFunc.getDateFormatter=function(){return dvxCharts.DateFormatter;};jFunc.getDateFormat=function(){return dvxCharts.DateFormatter.DateFormat;};jFunc.mouseCapture=function(el,params){var sharedData={};el.addEventListener('mousedown',function(e){var moveHandler;if(params.move){moveHandler=function(e){params.move.call(el,e);};document.addEventListener('mousemove',moveHandler);}
...
Tool.prototype={canStart:function(e){return false;},start:function(){},keyDown:function(e){},keyUp:function(e){},mouseMove:function(e){},mouseDown:function(e){},mouseUp:function(e){},touchStart:function(e){},touchMove:function(e){},touchEnd:function(e){},mouseWheel:function(e,delta,deltaX,deltaY){},stop:function(){},stopTool:function(){if(this.view.currentTool==this){this.view.setCurrentTool(null);}}}
function DefaultTool(view){Tool.call(this,view);}
DefaultTool.prototype=new Tool();DefaultTool.constructor=DefaultTool;DefaultTool.prototype.mouseDown=function(e){var tools=this.view.mouseDownTools;for(var i=0;i<tools.length;i++){var tool=tools[i];if(tool.canStart(e)){this.view.setCurrentTool(tool);return;}}}
DefaultTool.prototype.mouseMove=function(e){var tools=this.view.mouseMoveTools;for(var i=0;i<tools.length;i++){var tool=tools[i];if(tool.canStart(e)){this.view.setCurrentTool(tool);return;}}
this.view._processMouseMove(e);}
DefaultTool.prototype.touchStart=function(e){var tools=this.view.touchStartTools;for(var i=0;i<tools.length;i++){var tool=tools[i];if(tool.canStart(e)){this.view.setCurrentTool(tool);return;}}
this.view._processTouchStart(e);}
DefaultTool.prototype.touchMove=function(e){var tools=this.view.touchMoveTools;for(var i=0;i<tools.length;i++){var tool=tools[i];if(tool.canStart(e)){this.view.setCurrentTool(tool);return;}}
this.view._processTouchMove(e);}
function SvgControl(options){this.shapes=[];this.mouseDownTools=[];this.mouseMoveTools=[];this.touchStartTools=[];this.touchMoveTools=[];this.defaultTool=new DefaultTool(this);this.currentTool=this.defaultTool;this._events={};this.originalCursor='auto';if(jFunc.isPlainObject(options)){return;}
...
else if(!jFunc.height(domElement)){domElement.style.height=ops.defaultHeight||defHeight;}},_createSvg:function(){var svg=document.createElementNS("http://www.w3.org/2000/svg",'svg');svg.setAttribute('class',this.pluginClass+'-svg');this.getCanvasContainer().appendChild(svg);return svg;},_createTooltip:function(){var canvasContainer=this.getCanvasContainer(),tooltip;canvasContainer.insertAdjacentHTML('beforeend','<div style="position:absolute;display:none"></div>');tooltip=canvasContainer.lastChild;return tooltip;},_addTrialWatermark:function(shapes){var indexOf=window.location.host.indexOf('www.dvxcharts.com');if(indexOf!==-1){return;}
...
this.touchInput=input;},resetCursor:function(){if(this.getCursor()!==this.originalCursor){this.setCursor(this.originalCursor);}},getCursor:function(){return this.getDOMElement().style.cursor;},setCursor:function(cursor){if(!this.keepCursor){this.getDOMElement().style.cursor=cursor;}},_resize:function(){var domElement=this.getDOMElement();var w=jFunc.width(domElement);var h=jFunc.height(domElement);if(w===this._width&&h===this._height){return;}
...
this._oldShape=null;this.hideTooltip();this._clearHighlightShapes();},_clearHighlightShapes:function(){if(this.hlShapes){this.hlShapes.forEach(function(item){item.remove();});}},_processMouseMove:function(e){this._processTooltips(this.mouseInput);},_processMouseEvents:function(){var input=this.mouseInput;if(!input){return;}
...
var event={name:'tooltipFormat'};this.triggerEvent(event,[context]);var chart=this;if(event.result===false){this.hideTooltip();return false;}
...
this._clearHighlightShapes();var op=this.options.tooltips;var newShapes=[];jFunc.each(shapes,function(index,shape){if(shape.isHighlightShape){newShapes.push(shape);}});var context;if(newShapes.length==1){context=newShapes[0].context;context.shape=newShapes[0];}
else{context=[];jFunc.each(newShapes,function(){context.push(this.context);context.shape=this;});}
var event={name:'dataHighlighting'};this.triggerEvent(event,[context]);if(event.result!==false){this.hlShapes=newShapes;newShapes.forEach(function(item){item.renderToSvg(this.shapeRenderer.svg);}.bind(this));}},_getClosestShape:function(shapes,area,input){var result=shapes[0];for(var i=1;i<shapes.length;i++){var shape=shapes[i];if(Shape.compare(result,shape,input,area)==false){result=shape;}}
...
if(eventListeners.length==0){delete eventListeners[eventName];}
...
if(!element){return;}
var pClass=this.pluginClass;element.insertAdjacentHTML('beforeend','<div class="'+pClass+'"></div>');this.elem=element.lastChild;this._createElements();this.setState(this.getState());}.bind(this));}}
function AxisBase(options){this._initDefs(options);this.setOptions(options);}
...
var origin1=sh1.getCenter(input);var origin2=sh2.getCenter(input);var dx=input.locX-origin1.x;var dy=input.locY-origin1.y;var dist1=Math.sqrt(dx*dx+dy*dy);dx=input.locX-origin2.x;dy=input.locY-origin2.y;var dist2=Math.sqrt(dx*dx+dy*dy);return dist1<=dist2;};Shape.prototype.addClass=function(newClass){var newClasses=newClass.split(' '),that=this;newClasses.forEach(function(item){if(that.classes.indexOf(item)===-1){that.classes.push(item);}});};Shape.prototype.addClasses=function(classes){var that=this;classes.forEach(function(item){if(that.classes.indexOf(item)===-1){that.classes.push(item);}});};Shape.prototype.renderClasses=function(svgShape){if(this.classes.length){svgShape.setAttribute('class',this.classes.join(' '));}};Shape.prototype.hitTest=function(x,y,area){return false;};Shape.prototype.boundsHitTest=function(x,y,area){if(!this.useHitTestArea){area=0;}
...
polyline.setAttribute('points',polylinePoints);svg.appendChild(polyline);}
...
var test=this.boundsHitTest(x,y,area);if(test==false){return false;}
...
return oddNodes;}
Polygon.prototype.renderToSvg=function(svg){if(!this.visible){return;}
...
this.renderClasses(polygon);polygon.setAttribute('points',polygonPoints);svg.appendChild(polygon);this.svgShape=polygon;}
function Curve(pts,closed){PolyShape.call(this);this.decorationType="stroke";if(!pts){return;}
...
svg.appendChild(svgText);this.renderClasses(svgText);return svgText;};TextBlock.prototype.renderBg=function(ctx){};TextBlock.prototype._correctXY=function(x,y,w,h){switch(this.textAlign){case 'center':x-=w/2;break;case 'right':x-=w;break;}
...
this.shapes.forEach(function(item){item.renderToSvg(svgGroup);});this.renderClasses(svgGroup);svg.appendChild(svgGroup);};function ShapeRenderer(svg,chart){this.svg=svg;this.chart=chart;}
...
return false;}
ZoomingTool.prototype.start=function(){this.view._clearRenderers();this.zoomableAxes=this.view.axes.getZoomableAxes();this.mouseInput1=this.view.mouseInput;var cursor;if(this.view.canZoomHor&&this.view.canZoomVer){cursor='crosshair';}
else if(this.view.canZoomHor){cursor='e-resize';}
...
ZoomingTool.prototype.stop=function(){this.zoomableAxes=null;this.view._clearSelectionRect();this.view.resetCursor();Tool.prototype.stop.call(this);}
...
if(this.view.canZoom){e.preventDefault();return true;}
return false;}
PanningTool.prototype.start=function(){this.zoomableAxes=this.view.axes.getZoomableAxes();this.oldMouseInput=this.view.mouseInput;this.view.setCursor('move');}
...
PanningTool.prototype.stop=function(){this.zoomableAxes=null;this.view.resetCursor();Tool.prototype.stop.call(this);}
...
var defOptions={marker:{type:'rectangle'}};opt=jFunc.extend(true,{},defOptions,opt||{});var marker=new Marker(opt.marker);opt.marker=marker;var newOpt={};jFunc.extend(newOpt,opt);newOpt.text=opt.text.text;newOpt.context={chart:this.chart,title:newOpt.text};newOpt.cursor=newOpt.cursor||this.itemsCursor;var legendItem=new LegendItem(newOpt);legendItem.chart=this.chart;this.items.push(legendItem);}}
else{var cursor=this.itemsCursor;if(this.allowHideSeries){cursor=cursor||'pointer';}
var itemOptions={};var series=this.chart.series.items;for(var i=0;i<series.length;i++){var ser=series[i];if(!ser.visible){continue;}
...
this.width=size.width+this.marker.size+this.lblMargin;this.height=size.height;};LegendItem.prototype._arrange=function(){var r=this.marker.size/2;var x=this.x+r;var y=this.y+r+(this.height-this.marker.size)/2;this.markerShape=this.marker.getShape(x,y,r);this.markerShape.context=this.context;this.markerShape.cursor=this.cursor;this.markerShape.isLegendItem=true;this.setMarkerClasses(this.markerShape);this.textBlock.x=this.x+this.marker.size+this.lblMargin;this.textBlock.y=this.y;};LegendItem.prototype._render=function(shapes,seriesGroup,markersGroup){var elems=[];markersGroup=this.individualFill?markersGroup:new GroupShape();markersGroup.addShape(this.markerShape);this.setMarkersGroupClasses(markersGroup);if(this.individualFill){this.series.setSeriesClasses(seriesGroup);}else{elems.push(markersGroup);}
...
GridArea.prototype._renderSeries=function(serShapesRenderContext){var clipId='gridAreaClip';var seriesGroup=new GroupShape();seriesGroup.clipId=clipId;var series=jMath.sortSeries(this.chart.series.items);for(var i=0;i<series.length;i++){var ser=series[i];if(ser.notInGridArea){continue;}
...
PieSeries.prototype.setOptions=function(options){Series.prototype.setOptions.call(this,options);if(this.allowExplodeSlices){this.cursor=this.cursor||'pointer';}}
...
high=vl[1];low=vl[2];open=vl[3];close=vl[4];var context={chart:this.chart,series:this,dataItem:vl,index:i,x:this.realXAxis._getValue(dataX),high:high,low:low,open:open,close:close};x=this.realXAxis.getPosition(x);high=this.realYAxis.getPosition(high);low=this.realYAxis.getPosition(low);open=this.realYAxis.getPosition(open);close=this.realYAxis.getPosition(close);var sh=this._createShape(x,high,low,open,close,width);sh.context=context;this._addShapeAnimation(sh,i,len);renderContext.shapes.push(sh);}}
