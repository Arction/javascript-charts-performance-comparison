/*
 * dvxCharts v5.1.0.0 (http://www.dvxcharts.com)
 * Copyright 2022 dvxCharts Inc. All rights reserved.
 *
...
if(SvgControl.isMouseDevice){jFunc.bind(domElement,'mouseenter',function(e){that._mouseEnter(e);});jFunc.bind(domElement,'mouseleave',function(e){that._mouseLeave(e);});jFunc.mouseCapture(domElement,{down:jFunc.proxy(that._mouseDown,this),move:jFunc.proxy(that._mouseMove,this),up:jFunc.proxy(that._mouseUp,this)});jFunc.bind(domElement,'mousemove',function(e){if(!that.isMouseDown){that._mouseMove(e);}});}
...
else{this._setOptions(this.options);}},_clearRenderers:function(){this._clearHightlightShapes();if(this._oldTShapes){this._oldTShapes=null;this.triggerEvent('dataHighlighting',null);}
...
this._clearHightlightShapes();var area=this.options.tooltips.snapArea;var newShapes=this._getTooltipShapes(input.locX,input.locY,area,input);var different=true;if(this._oldTShapes==null||!jMath.compareArrays(this._oldTShapes,newShapes)){if(newShapes){different=this._initTooltip(newShapes);this._highlightShapes(newShapes);}
...
return result;},_getTooltip:function(shape){return "Tooltip";},_clearHightlightShapes(){(this._oldTShapes||[]).forEach(function(shape){if(shape.isHighlightShape){shape.remove();}});},_getTooltipText:function(shapes){var text='';var that=this;jFunc.each(shapes,function(){var itemText=that._getTooltip(this);if(itemText){text+=itemText;}});return text;},_getTooltipShapes:function(x,y,area,input){return this._findClosestShape(this.shapes,x,y,area,input);},_findClosestShape:function(inputShapes,x,y,area,input){var shapes=[],shape,result,sh,i;if(!area){area=0;}
...
return intrStart;},_alignToInterval:function(value,interval){return jMath.round(jMath.round(Math.floor(value/interval))*interval)+this.getCrossing();},_createLabel:function(text,settings){var tBlock=new TextBlock(text);tBlock.chart=this.chart;tBlock.addClass('dvx-chart-axis-label');tBlock.isAxisLabel=true;tBlock.context={chart:this.chart,axis:this,text:text};jFunc.extend(tBlock,settings);this.chart.triggerEvent('axisLabelCreating',tBlock);tBlock.measure([this.getClasses(),['dvx-chart-axis-label-group']]);return tBlock;},_getLabelIntervals:function(interval,mark){return this._getIntervals(interval,mark);},_measureRotatedLabels:function(labels){var isVertical=this.isAxisVertical;var w=0;var h=0;for(var i=0;i<labels.length;i++){var lbl=labels[i];var hypotenuse=Math.sqrt(lbl.width*lbl.width+lbl.height*lbl.height);var rotAngle=lbl.rotationAngle;if(isVertical){var width=Math.abs(Math.cos(rotAngle)*hypotenuse);w=Math.max(w,width);}
...
this.title._measure([this.getClasses(),['dvx-chart-axis-title-group']]);var offset=this.title.height+rangeSliderBreadth+this.lineWidth/2;if(this.isAxisVertical){size.w+=offset;}
else{size.h+=offset;}
...
if(!isANull&&isBNull){return 1;}
return a.zIndex-b.zIndex;});return sortedSeries;};jMath.parse=function(str){var signs=[".","*","/","+","-"],funcs=[dot,multiply,divide,add,substract],tokens=str.split(/\b/),round,place,a,b,result;for(round=0;round<signs.length;round++){for(place=0;place<tokens.length;place++){if(tokens[place]===signs[round]){a=parseFloat(tokens[place-1]);b=parseFloat(tokens[place+1]);result=funcs[round](a,b);tokens[place-1]=result.toString();tokens.splice(place--,2);}}}
return parseFloat(tokens[0]);function dot(x,y){return parseFloat(x+"."+y);}
function multiply(x,y){return x*y;}
function divide(x,y){return x/y;}
function add(x,y){return x+y;}
function substract(x,y){return x-y;}};function jDate(){}
...
var origin1=sh1.getCenter(input);var origin2=sh2.getCenter(input);var dx=input.locX-origin1.x;var dy=input.locY-origin1.y;var dist1=Math.sqrt(dx*dx+dy*dy);dx=input.locX-origin2.x;dy=input.locY-origin2.y;var dist2=Math.sqrt(dx*dx+dy*dy);return dist1<=dist2;};Shape.prototype.getClasses=function(){return this.classes;};Shape.prototype.addClass=function(newClass){this.addClasses(newClass.split(' '));};Shape.prototype.addClasses=function(classes){var that=this;(classes||[]).forEach(function(item){if(that.classes.indexOf(item)===-1){that.classes.push(item);if(that.svgShape){that.svgShape.classList.add(item);}}});};Shape.prototype.removeClass=function(newClass){this.removeClasses(newClass.split(' '));};Shape.prototype.removeClasses=function(classes){var that=this;classes.forEach(function(item){var indexOf=that.classes.indexOf(item);if(indexOf>0){that.classes.splice(indexOf,1);if(that.svgShape){that.svgShape.classList.remove(item);}}});};Shape.prototype.renderClasses=function(svgShape){if(this.classes.length){svgShape.setAttribute('class',this.classes.join(' '));}};Shape.prototype.hitTest=function(x,y,area){return false;};Shape.prototype.boundsHitTest=function(x,y,area){if(!this.useHitTestArea){area=0;}
...
return svg;};TextBlock.prototype.measure=function(parents){var svg=this.chart.svg,svgText,bBox,rootGroup,parentGroup=svg,svgGroup=svg;(parents||[]).forEach(function(classes){svgGroup=document.createElementNS("http://www.w3.org/2000/svg",'g');svgGroup.setAttribute('class',classes.join(' '));parentGroup.appendChild(svgGroup);parentGroup=svgGroup;if(!rootGroup){rootGroup=svgGroup;}});svgText=this.renderToSvg(svgGroup,true);bBox=svgText.getBBox();svg.removeChild(rootGroup||svgText);this.width=bBox.width;this.height=bBox.height;this.dy=bBox.y;return{width:bBox.width,height:bBox.height};};TextBlock.prototype.renderToSvg=function(svg,measuring){if(!this.visible||this.text===undefined){return;}
...
return this.renderText(svg,x,y,measuring);};TextBlock.prototype.renderText=function(svg,x,y,measuring){var text=''+this.text,svgText=document.createElementNS("http://www.w3.org/2000/svg",'text');svgText.setAttribute('x',x);svgText.setAttribute('y',y);svgText.textContent=text;if(this.rotationAngle){svgText.setAttribute('transform','rotate('+jMath.degrees(this.rotationAngle)+')');svgText.setAttribute('transform-origin',this.rotX+' '+this.rotY);}
this.renderClasses(svgText);if(measuring){svgText.classList.add('dvx-element-hidden');}
svg.appendChild(svgText);this.svgShape=svgText;return svgText;};TextBlock.prototype._correctXY=function(x,y,w,h){switch(this.textAlign){case 'center':x-=w/2;break;case 'right':x-=w;break;}
...
Title.prototype._measure=function(parents){var size;if(!this.text){this.width=0;this.height=0;return;}
this.textBlock.chart=this.chart;size=this.textBlock.measure(parents);var l=2*this.margin;this.width=size.width+l;this.height=size.height+l;}
...
GridLine.prototype.setOptions=function(options){var settings=jFunc.extend({},this.defaults,options||{});jFunc.extend(this,settings);};function PlotLine(options){this.defaults={zIndex:0,title:{margin:2,hAlign:'left',vAlign:'top',svgClass:'dvx-chart-axis-plot-line-title',orientation:'horizontal'}};this.setOptions(options);}
...
var settings=jFunc.extend(true,{},this.defaults,options||{});jFunc.extend(this,settings);}};function PlotBand(options){this.defaults={zIndex:0,title:{margin:2,hAlign:'left',vAlign:'top',svgClass:'dvx-chart-axis-plot-band-title',orientation:'horizontal'}};this.setOptions(options);}
...
var paddings=this.paddings=this.border.getPaddings();this.title._measure([['dvx-chart-legend-group']]);var w=this.title.width;var h=this.title.height;if(this.title.text){h+=paddings.top;}
var isHorizontal=this._isHorizontal();var itemsHorMargin=this.itemsHorMargin;var itemsVerMargin=this.itemsVerMargin;var itemsWidth=0;var itemsHeight=0;var maxItemWidth=0;for(var i=0;i<this.items.length;i++){var item=this.items[i];item._measure(['dvx-chart-legend-group']);if(isHorizontal){itemsWidth+=item.width;if(i>0){itemsWidth+=itemsHorMargin;}
...
GridArea.prototype._arrange=function(){var x=this.x;var y=this.y;this.border.x=x;this.border.y=y;this.border.width=this.width;this.border.height=this.height;};GridArea.prototype._renderZIndex=function(axesGroup,zIndex){this._renderPlots(axesGroup,zIndex);};GridArea.prototype._render=function(shapes){this.border._render(shapes);var axesGroup=new GroupShape();shapes.push(axesGroup);this._renderPlots(axesGroup,0);this._renderGridLines(axesGroup);this._renderZIndex(axesGroup,1);};GridArea.prototype._renderSeries=function(serShapesRenderContext){var clipId=this.chart.dvxId+'_gaClip';var seriesGroup=new GroupShape();seriesGroup.clipId=clipId;var series=jMath.sortSeries(this.chart.series.items);for(var i=0;i<series.length;i++){var ser=series[i];if(ser.notInGridArea){continue;}
...
var clip=new Clip(clipId,this.x,this.y,this.width,this.height);serShapesRenderContext.shapes.push(clip);};GridArea.prototype._renderStripes=function(shapes){var axes=this.chart.axes.items;for(var i=0;i<axes.length;i++){var axis=axes[i];var stripes=this._getStripes(axis);jFunc.merge(shapes,stripes);}};GridArea.prototype._renderPlots=function(axesGroup,zIndex){var axes=this.chart.axes.items;for(var i=0;i<axes.length;i++){var axis=axes[i];var plotBandsGroup=this._getPlotBands(axis,zIndex);if(plotBandsGroup){axesGroup.addShape(plotBandsGroup);}
var plotLinesGroup=this._getPlotLines(axis,zIndex);if(plotLinesGroup){axesGroup.addShape(plotLinesGroup);}}};GridArea.prototype._renderGridLines=function(axesGroup){var axes=this.chart.axes.items;for(var i=0;i<axes.length;i++){var axis=axes[i];var axisGroupShape=new GroupShape();axisGroupShape.addClasses(axis.getClasses());var majorGridLines=axis.majorGridLines;if(!majorGridLines&&axis.getOrientation()==='y'){majorGridLines=new GridLine();if(axis.minorGridLines){axis.minorGridLines.major=majorGridLines;}}
...
axesGroup.addShape(axisGroupShape);}};GridArea.prototype._getStripes=function(axis){var stripes=axis.stripes;if(stripes==null||jFunc.isArray(stripes)!=true){return[];}
var gArea=this;var shapes=[];jFunc.each(stripes,function(){var stripe=new Stripe(this);var intr;if(stripe.interval){intr=stripe.interval;}else{intr=2*axis.actualInterval;}
var width;if(stripe.width){width=stripe.width;}else{width=axis.actualInterval;}
...
var pos1=axis.getPosition(val);var next=axis._getNextPosition(val,width);var pos2=axis.getPosition(next);var rect;if(axis.isAxisVertical){rect=new Rectangle(gArea.x+lw2,Math.min(pos1,pos2),gArea.width-lineWidth,Math.abs(pos2-pos1));}else{rect=new Rectangle(Math.min(pos1,pos2),gArea.y+lw2,Math.abs(pos2-pos1),gArea.height-lineWidth);}
stripe._setSettings(rect);shapes.push(rect);}});return shapes;};GridArea.prototype._getGridLines=function(axis,mark,major){if(axis.location==='radial'||!mark||mark.visible!==true){return;}
var intr=axis._getMarkInterval(mark,major);var intrs=axis._getIntervals(intr,mark,major);var x1,y1,x2,y2,svgClass=major?'dvx-chart-axis-major-grid-line':'dvx-chart-axis-minor-grid-line';var groupShape=new GroupShape();groupShape.addClass(svgClass);for(var i=0;i<intrs.length;i++){var pos=axis.getPosition(intrs[i]);if(axis.isAxisVertical){y1=y2=pos;x1=this.x;x2=x1+this.width;}else{x1=x2=pos;y1=this.y;y2=y1+this.height;}
var line=new Line(x1,y1,x2,y2);groupShape.addShape(line);}
return groupShape;};GridArea.prototype._getPlotLines=function(axis,zIndex){var plotLines=axis.plotLines;if(!plotLines||!jFunc.isArray(plotLines)){return null;}
var plotLinesGroup=new GroupShape();plotLinesGroup.addClasses(axis.getClasses());var gArea=this;var x1,y1,x2,y2;for(var i=0;i<plotLines.length;i++){var plotLine=new PlotLine();plotLine.setOptions(plotLines[i]);if(jMath.isNull(plotLine.value)){continue;}
if(jMath.fitInRange(plotLine.zIndex,0,2)!==zIndex){continue;}
if(axis.actualVisibleMinimum>plotLine.value||axis.actualVisibleMaximum<plotLine.value){continue;}
var plotLineGroup=new GroupShape();plotLineGroup.addClass('dvx-chart-axis-plot-line-group');if(plotLine.class){plotLineGroup.addClass(plotLine.class);}
plotLinesGroup.addShape(plotLineGroup);var lineWidth=2;var pos=axis.getPosition(plotLine.value);var title=new Title(plotLine.title);title.chart=this.chart;title._measure([axis.getClasses(),plotLineGroup.getClasses()]);if(axis.isAxisVertical){y1=y2=pos;x1=this.x;x2=x1+this.width;}else{x1=x2=pos;y1=this.y;y2=y1+this.height;}
var isTitleHorizontal=title.orientation==='horizontal';var titleRotationAngle=isTitleHorizontal?0:90;var titleOffsetCorr=isTitleHorizontal?0:4;if(isTitleHorizontal){title.x=x1;title.y=y1;if(axis.isAxisVertical){switch(title.hAlign){case 'center':title.x=gArea.x+(gArea.width-title.width)/2;break;case 'right':title.x=gArea.x+gArea.width-title.width;break;}
switch(title.vAlign){case 'bottom':title.y+=lineWidth;break;case 'center':title.y-=title.height/2;break;case 'top':title.y-=title.height+lineWidth;break;}}else{switch(title.hAlign){case 'left':title.x+=lineWidth;break;case 'center':title.x-=title.width/2;break;case 'right':title.x-=title.width+lineWidth;break;}
switch(title.vAlign){case 'center':title.y+=(gArea.height-title.height)/2;break;case 'bottom':title.y+=gArea.height-title.height;break;}}}else{title.x=x1;title.y=y1+title.height;if(axis.isAxisVertical){switch(title.hAlign){case 'center':title.x+=(gArea.width-title.height)/2;break;case 'right':title.x+=gArea.width-title.height;break;}
switch(title.vAlign){case 'top':title.y-=title.width+titleOffsetCorr;break;case 'center':title.y-=title.width/2;break;}}else{switch(title.hAlign){case 'center':title.x-=title.height/2;break;case 'right':title.x-=title.height;break;}
switch(title.vAlign){case 'center':title.y+=(gArea.height-title.width)/2;break;case 'bottom':title.y+=gArea.height-title.width-titleOffsetCorr;break;}}}
title.rotX=title.x+title.height-title.margin;title.rotY=title.y+title.margin;title.rotationAngle=jMath.radians(titleRotationAngle);var line=new Line(x1,y1,x2,y2);line.addClass("dvx-chart-axis-plot-line");plotLineGroup.addShape(line);var titleShapes=[];title._render(titleShapes);if(titleShapes.length){plotLineGroup.addShapes(titleShapes);}}
return plotLinesGroup;};GridArea.prototype._getPlotBands=function(axis,zIndex){var plotBands=axis.plotBands;if(!plotBands||!jFunc.isArray(plotBands)){return null;}
var gArea=this;var plotBandsGroup=new GroupShape();plotBandsGroup.addClasses(axis.getClasses());for(var i=0;i<plotBands.length;i++){var plotBand=new PlotBand();plotBand.setOptions(plotBands[i]);if(jMath.isNull(plotBand.from)||jMath.isNull(plotBand.to)){continue;}
if(jMath.fitInRange(plotBand.zIndex,0,2)!==zIndex){continue;}
if(axis.actualVisibleMinimum>plotBand.to||axis.actualVisibleMaximum<plotBand.from){continue;}
var plotBandGroup=new GroupShape();plotBandGroup.addClass('dvx-chart-axis-plot-band-group');if(plotBand.class){plotBandGroup.addClass(plotBand.class);}
plotBandsGroup.addShape(plotBandGroup);var pos1=axis.getPosition(Math.max(plotBand.from,axis.actualVisibleMinimum));var pos2=axis.getPosition(Math.min(plotBand.to,axis.actualVisibleMaximum));var x,y,w,h;if(axis.isAxisVertical){x=gArea.x;y=Math.min(pos1,pos2);w=gArea.width;h=Math.abs(pos2-pos1);}else{x=Math.min(pos1,pos2);y=gArea.y;w=Math.abs(pos2-pos1);h=gArea.height;}
var title=new Title(plotBand.title);title.chart=this.chart;title._measure([axis.getClasses(),plotBandGroup.getClasses()]);var isTitleHorizontal=title.orientation==='horizontal';var titleRotationAngle=isTitleHorizontal?0:90;var titleOffsetCorr=isTitleHorizontal?0:4;if(isTitleHorizontal){title.x=x;title.y=y;switch(title.hAlign){case 'center':title.x+=(w-title.width)/2;break;case 'right':title.x+=w-title.width;break;}
switch(title.vAlign){case 'center':title.y+=(h-title.height)/2;break;case 'bottom':title.y+=h-title.height;break;}}else{title.x=x;title.y=y+title.height;switch(title.hAlign){case 'center':title.x+=(w-title.height)/2;break;case 'right':title.x+=w-title.height;break;}
switch(title.vAlign){case 'center':title.y+=(h-title.width)/2;break;case 'bottom':title.y+=h-title.width-titleOffsetCorr;break;}}
title.rotX=title.x+title.height-title.margin;title.rotY=title.y+title.margin;title.rotationAngle=jMath.radians(titleRotationAngle);var rect=new Rectangle(x,y,w,h);rect.addClass("dvx-chart-axis-plot-band");plotBandGroup.addShape(rect);var titleShapes=[];title._render(titleShapes);if(titleShapes.length){plotBandGroup.addShapes(titleShapes);}}
return plotBandsGroup;};GridArea.prototype._contains=function(x,y){return x>=this.x&&x<=this.x+this.width&&y>=this.y&&y<=this.y+this.height;};GridArea.prototype.getRight=function(){return this.x+this.width;};GridArea.prototype.fitHor=function(x){return jMath.fitInRange(x,this.x,this.getRight());};GridArea.prototype.fitVer=function(y){return jMath.fitInRange(y,this.y,this.y+this.height);};function SeriesArray(chart,options){this.chart=chart;if(options){this.setOptions(options);}}
...
var lblSize;if(this.labels&&this.labels.visible!==false){var tBlock=new TextBlock('TEST');tBlock.chart=this.chart;jFunc.extend(tBlock,this.labels);lblSize=tBlock.measure();}
...
return vl;},_getDataPointLabel:function(vl,x,y,offset,dataValue,index){var isTop=vl>=this.realYAxis.getCrossing();var text=this._getLabelText(dataValue);var tBlock=new TextBlock(text);tBlock.chart=this.chart;this.setLabelClasses(tBlock,index);this.setShadows(tBlock);jFunc.extend(tBlock,this.labels);tBlock.measure([this.getClasses(),this.getLabelGroupClasses()]);tBlock.textAlign='center';tBlock.x=x;if(isTop){tBlock.y=y-offset;tBlock.textBaseline='bottom';}
...
axis.chart=this.chart;return axis;};BarSeries.prototype._createYAxis=function(){var axis=new LinearAxis({location:'bottom',orientation:'y'});axis.chart=this.chart;return axis;};BarSeries.prototype._findXAxis=function(axes){var axis=this._findAxis(axes,this.axisX);if(axis!=null){return axis;}
...
return null;};BarSeries.prototype._findYAxis=function(axes){var axis=this._findAxis(axes,this.axisY);if(axis!=null){return axis;}
...
var locY=y-h/2;var rect=new Rectangle(x,locY,width,h);rect.context=context;rect.center={x:Math.round(mX),y:Math.round(locY+h/2)};this._setShapeSettings(rect,i);this.setItemClasses(rect,i);renderContext.shapes.push(rect);this._addAnimation(rect,i,len);this._addMarkerAndLabel(renderContext,mX,y,i,len,vlX,vlY,lblOffset,context);}};BarSeries.prototype._addAnimation=function(rect,i,len){var anOptions=this._getAnimation();if(!anOptions||anOptions.enabled===false){return;}
var animation=new Animation(anOptions,rect,"xDecrease",rect.width,0);this._setIsAnimReversed(rect);var duration=animation.duration/len;var delayTime=animation.delayTime+i*duration;animation.delayTime=delayTime;animation.duration=duration;this.chart.storyboard.addAnimation(animation);};BarSeries.prototype._setIsAnimReversed=function(rect){var rectReversed=rect.context.y<this.realYAxis.getCrossing();var reversed=this.realYAxis.reversed;rect.isAnimReversed=(!reversed&&rectReversed)||(reversed&&!rectReversed);};BarSeries.prototype._correctMarkerPosition=function(x,y,vl){var offset=this.markers.offset;if(offset){var isTop=vl>=this.realYAxis.getCrossing();x=isTop?x+offset:x-offset;}
return{x:x,y:y};};BarSeries.prototype._getPixelMargins=function(axis){if(axis.isVertical()==false){var markerMargin=Series.prototype._getPixelMargins.call(this,axis);var margin=axis.length/10;var left=Math.max(margin,markerMargin.left);var right=Math.max(margin,markerMargin.right);return{left:left,right:right};}
if(!this.hasData()){return{left:0,right:0};}
var offset=4;var series=this._getSeriesFromThisType();var columnScale=this._calcColumnScale(series);var length=axis.length;var width=0.5*columnScale*length+offset;return{left:width,right:width};};BarSeries.prototype._isAnchoredToOrigin=function(){return true;};BarSeries.prototype._getDataPointLabel=function(vl,x,y,offset,dataValue,index){var isLeft=vl<this.realYAxis.getCrossing();var text=this._getLabelText(dataValue);var tBlock=new TextBlock(text);tBlock.chart=this.chart;this.setLabelClasses(tBlock,index);this.setShadows(tBlock);jFunc.extend(tBlock,this.labels);tBlock.measure([this.getClasses(),this.getLabelGroupClasses()]);tBlock.y=y;var gArea=this.chart.gridArea;if(isLeft){tBlock.x=x-offset;tBlock.textAlign='right';if(tBlock.x-tBlock.width<gArea.x+4){tBlock.x=gArea.x+tBlock.width+4;}}
...
if(vl==0){continue;}
var text=this._getLabelText(labelValue);var angle=(startAngle+endAngle)/2;var textBlock=new TextBlock(text);textBlock.chart=this.chart;textBlock.textBaseline='top';this.setLabelClasses(textBlock,i);jFunc.extend(textBlock,this.labels);textBlock.context={chart:this.chart,series:this,dataItem:data[i],index:i,value:vl,percentage:percentage};this.chart.triggerEvent('dataPointLabelCreating',textBlock);var size=textBlock.measure([this.getClasses(),this.getLabelGroupClasses()]);var cx=x;var cy=y;exploded=this.isExploded(i);if(exploded){cy=y+explodedRadius*Math.sin(angle);cx=x+explodedRadius*Math.cos(angle);}
...
RangeSeries.prototype=new Series();RangeSeries.constructor=RangeSeries;RangeSeries.prototype._initXYData=function(){this._initXYDataRange(1,3);};RangeSeries.prototype._initCatValueData=function(){this._initCatValueDataRange(1,3,true);};RangeSeries.prototype._initDateValueData=function(){this._initDateValueDataRange(1,3);};RangeSeries.prototype._initVisibleCatValueData=function(){this._initVisibleCatValueDataRange(1,3);};RangeSeries.prototype._initVisibleXYData=function(){this._initVisibleXYDataRange(1,3);};RangeSeries.prototype._processData=function(){this._processDataXYZ();};RangeSeries.prototype._renderCatData=function(renderContext){var data=this.arrData;var len=data.length;var hasMarkers=this.markers!=null&&this.markers.isVisible();var pts1=[];var pts2=[];var cPoints=[];var lblOffset=this.getLabelsOffset();var x,y1,y2,vlX,vlY1,vlY2;for(var i=0;i<len;i++){var vl=data[i];if(vl===null||vl===undefined){pts1.push(null);pts1.push(null);pts2.push(null);pts2.push(null);if(!hasMarkers){cPoints.push(null);}
continue;}
...
this._addMarkersAndLabels(renderContext,x,y1,y2,i,len,null,vlY1,vlY2,lblOffset);}
var polyline=this._createShape(pts1,pts2,renderContext);if(!hasMarkers&&polyline){polyline.context={chart:this.chart,series:this,points:cPoints};}
else{}};RangeSeries.prototype._createShape=function(pts1,pts2,renderContext){var polygon=new RangeShape(pts1,pts2);this._setShapeSettings(polygon);renderContext.shapes.push(polygon);this._addLengthAnimation(polygon);return polygon;};RangeSeries.prototype._addMarkersAndLabels=function(renderContext,x,y1,y2,i,length,vlX,vlY1,vlY2,lblOffset){var context;var mOffset=0;var data=this.arrData;var dataX=vlX?vlX:i;if(this.markers&&this.markers.isVisible()){context={chart:this.chart,series:this,dataItem:data[i],index:i,x:this.realXAxis._getValue(dataX),from:vlY1,to:vlY2};mOffset=this.markers.offset;var imagePath=context.dataItem[3];var marker=this._addMarker(x,y1,null,vlY1,imagePath,i,context);if(marker.marker){if(marker.line){markers.push(marker.line);}
...
var cx=shapeBounds.x+shapeBounds.w/2,cy=shapeBounds.y+shapeBounds.h/2;var text=this._getLabelText(labelValue);var textBlock=new TextBlock(text);textBlock.chart=this.chart;this.setLabelClasses(textBlock,index);textBlock.textAlign='center';jFunc.extend(textBlock,this.labels);textBlock.measure([this.getClasses(),this.getLabelGroupClasses()]);textBlock.x=cx;textBlock.y=cy;textBlock.context=context;this.chart.triggerEvent('dataPointLabelCreating',textBlock);return textBlock;};FunnelSeries.prototype._getNextValueIndex=function(index){var data=this.arrData,hiddenSlices=this.hiddenSlices,length=data.length;for(var i=index+1;i<length;i++){if(jFunc.inArray(i,hiddenSlices)==-1){return i;}}
...
tBlocks.push(tBlock);}
return tBlocks;}
CategoryAngleAxis.prototype._render=function(renderContext){var classes=this.getClasses();var groupShape=new GroupShape();groupShape.addClasses(classes);var postGroupShape=new GroupShape();postGroupShape.addClasses(classes);renderContext.postShapes.push(postGroupShape);var len=this.actualMaximum;if(!len){return;}
